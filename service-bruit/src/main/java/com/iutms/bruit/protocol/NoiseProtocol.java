...
/**
 * Custom text-based TCP protocol for noise sensor data.
 * Format: NOISE|zoneId|decibelLevel|timestamp
...
public class NoiseProtocol {
    ...
    public static NoiseData decode(String message) {
        ...
        if (parts.length != 4 || !parts[0].equals(PREFIX)) {
            throw new IllegalArgumentException("Invalid noise message: " + message);
        }
